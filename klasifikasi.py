...
selected = option_menu(
    menu_title=None,
    options=['Data','Implementasi','Kelompok'],
...
if selected == 'Data':
    ...
    st.write('4. texture_mean = (deviasi standar nilai skala abu-abu)')
    st.write('5. perimeter_mean = (keliling)')
    st.write('6. area_mean = (luas)')
    st.write('7. smoothness_mean = (variasi lokal dalam panjang jari-jari)')
    st.write('8. compactness_mean = (keliling ^ 2 / luas - 1,0)')
    st.write('9. concavity_mean = (tingkat keparahan bagian cekung dari kontur)')
    st.write('10. concave_points_mean = (jumlah bagian cekung dari kontur)')
    st.write('11. symmetry_mean')
    st.write('12. fractal_dimension_mean = ("coastline approximation" - 1)')
    ...
    st.write('24. texture_worst')
    st.write('25. perimeter_worst')
    st.write('26. area_worst')
    st.write('27. smoothness_worst')
    st.write('28. compactness_worst')
    st.write('29. concavity_worst')
    st.write('30. concave points_worst')
    st.write('31. symmetry_worst')
    st.write('32. fractal_dimension_worst')
...
if selected == 'Implementasi':
    ...
    if button:
        if radius_mean !=0 and texture_mean !=0 and perimeter_mean !=0 and area_mean !=0 and smoothness_mean !=0 and compactness_mean!=0 and concavity_mean!=0 and concave_points_mean!=0 and symmetry_mean !=0 and fractal_dimension_mean !=0 and  radius_se!=0 and  texture_se!=0 and  perimeter_se!=0 and  area_se!=0 and  smoothness_se!=0 and compactness_se!=0 and  concavity_se!=0 and   concave_points_se!=0 and  symmetry_se!=0 and  fractal_dimension_se!=0 and  radius_worst!=0 and  texture_worst!=0 and  perimeter_worst!=0 and  area_worst!=0 and  smoothness_worst!=0 and  compactness_worst!=0 and  concavity_worst!=0 and  concave_points_worst!=0 and  symmetry_worst!=0 and  fractal_dimension_worst !=0 :
            ...
            #st.write(data_norm)
            with open('PCA_BC.pkl','rb') as pca15:
                pca =pickle.load(pca15)
            data_pca = pca.transform(data_norm)
            #st.write('---------- Data hasil Ekstraksi Fitur dengan PCA ----------')
            #st.write(data_pca)
            with open('svm_BC.pkl','rb') as pcasvm :
                svm_pca = pickle.load(pcasvm)
            predict_pca = svm_pca.predict(data_pca)
            if predict_pca==0:
